...
while($true)
{
	...
	else
	{
		...
		{
			Write-Warning "Main: ASF discovery has been attempted $retryCount times. Bootstrap will now exit"
			$global:state = [State]::TimedOut
			break
		}
	}
	
	Write-Host "Main: Attempt[$retryCount of $RetryMax]: ASF discovery failed. Wait $RetryWait secs and try again"
	sleep $RetryWait
}
...
{
	Write-Warning  "ASF discovery and callback execution process timedout: No services found."
}
