...
@(
  ...
  @{ name    =   'create_local_administrator';
      script = { New-LocalAdministrativeUser -Username "Administrator" -Password "tally.h0" -Verbose:$VerbosePreference };
      pre    = { if ( (Get-CurrentDomain).PartOfDomain ) { Throw "Computer is a domain member." } };
      post   = { 1 }; }
  # Backup administrator
  @{ name    =   'create_local_administrator_citrix';
      script = { New-LocalAdministrativeUser -Username "citrix" -Password "tally.h0" -Verbose:$VerbosePreference };
      pre    = { if ( (Get-CurrentDomain).PartOfDomain ) { Throw "Computer is a domain member." } };
      post   = { 1 }; }
  ...
  @{ name    =   'enable_automatic_administrative_login';
      script = { Set-AutoAdminLogon -DefaultUserName "Administrator" -DefaultPassword "tally.h0" -Verbose:$VerbosePreference };
      pre    = { 1 };
      post   = { 1 }; }
),
