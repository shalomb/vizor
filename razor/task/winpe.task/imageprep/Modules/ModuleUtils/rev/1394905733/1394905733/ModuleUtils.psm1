...
<#
TODO
  * Read PSD file in with comments
  * Make in-changes to particular keys while preserving all other information including comments
#>
...
<#
TODO
  * Dependency Resolution - DependencyResolver.psm1
#>
...
function Find-Module {
  ...
  Param(
    ...
    [Alias("BasePath")]
      [String]$BasePaths = $Env:PSModulePath
  )
  ...
  $OLDPWD = $PWD  # We are going to change directory
  $Seen = @{}
  $Candidates = @()
  $Candidates = $BasePaths -split ";" | %{
    if ( Test-Path ($Path = $_) ) {
...
.DESCRIPTION
Recursively scan a directory hierarchy and find all modules matching a given name.
...
function Set-PSModulePath {
  ...
  if ( $Scope -match '^Process' ) {
    ...
    Write-Host -Fore cyan "$PSModulePath"
    return
...
function Add-PSModulePath {
  ...
  if ($CurrentValue = Get-ModulePath -Scope $Scope) {
...
.NOTES  
  ...
  NOTES FOR DEVELOPERS
    This contains notes for developers who are interested in this module.
...
function Resolve-NodeDependencies {
  ...
  if ( $UnResolvedNodes.Value.Contains($NodeName) ) {
    $err = "$($MyInvocation.MyCommand.Name), FATAL: Potential circular dependency detected while resolving Node ($Node).`n"+
           "  The Node ($Node) was previously marked for resolution but not resolved."
    throw $err
  } else {
    $UnResolvedNodes.Value.Add($NodeName, 0)
  }
...
function New-Node {
  ...
  $Node = New-Object PSObject |
    ...
    Add-Member -PassThru -MemberType NoteProperty -Name Attributes  -Value $Attributes |
    Add-Member -PassThru -MemberType NoteProperty -Name Edges       -Value $Edges |
    Add-Member -PassThru -MemberType ScriptMethod -Name AddEdge     -Value {
      Param(
        [PSObject[]] $Edges    
      )
      $this.Edges += $Edges
